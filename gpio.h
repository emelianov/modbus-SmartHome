...
uint16_t cbDigitalRead(TRegister *reg, uint16_t val) {
  std::vector<gpio_t>::iterator it =
      std::find_if(gpios.begin(), gpios.end(),
                   [reg](const gpio_t &d) { return d.reg == reg->address; });
  if (it != gpios.end()) {
    switch (it->reg.type) {
    case TAddress::HREG:
      return 0;
...
uint16_t addGpio(TAddress local, uint8_t pin, uint8_t mode) {
  ...
  case TAddress::COIL:
    ...
    mb->onSetCoil(local.address,
                  (mode == OUTPUT) ? cbDigitalWrite : cbDigitalRead);
    mb->onGetCoil(local.address, cbDigitalRead);
    break;
  case TAddress::ISTS:
    mb->addIsts(local.address);
    mb->onSetIsts(local.address,
                  (mode == OUTPUT) ? cbDigitalWrite : cbDigitalRead);
    ...
    if (data) {
      if (configFile.read((uint8_t *)data, configFile.size()) ==
        ...
        if (json) {
          ...
          cJSON *item;
          i = 0;
          if (cJSON_HasObjectItem(json, "gpio"))
            arr = cJSON_GetObjectItemCaseSensitive(json, "gpio");
          if (arr) {
            ...
            cJSON_ArrayForEach(entry, arr) {
              ...
              TDEBUG("gpio: #%d\n", i);
              cJSON *item = NULL;
              if (!cJSON_HasObjectItem(entry, "type"))
                continue;
              ...
              if (!cJSON_HasObjectItem(entry, "inverse"))
                continue;
              ...
              if (!cJSON_HasObjectItem(entry, "mode"))
                continue;
...
ShellCommand(gpiomapists, "<pin> <local_ists> - GPIO: Map to Ists",
             cliGpioMapIsts);
...
ShellCommand(gpiomapcoil, "<pin> <local_coil> - GPIO: Map to Coil",
             cliGpioMapCoil);
...
void cliGpioMapList(Shell &shell, int argc, const ShellArguments &argv) {
  ...
  for (auto &s : gpios) {
    shell.printf_P(PSTR("%d\t %s \t%s\t%d\t'%s\n"), s.pin,
                   (s.mode == OUTPUT) ? "<=" : "=>", regTypeToStr(s.reg),
                   s.reg.address, s.name.c_str());
  }
}

...
struct sensor {
  ...
  String addressToString() {
    char szRet[24];
    sprintf(szRet, "%02X%02X%02X%02X%02X%02X%02X%02X", device[0], device[1],
...
bool saveSensors() {
  ...
  if (configFile) {
    ...
    if (parm)
      cJSON_AddItemToObject(json, "pin", parm);
    parm = cJSON_CreateNumber(queryInterval + DEVICE_RESPONSE_WAIT);
    if (parm)
      cJSON_AddItemToObject(json, "interval", parm);
  ...
  if (configFile) {
    ...
    if (data) {
      ...
      if (json) {
        ...
        if (cJSON_HasObjectItem(json, "sensors"))
          arr = cJSON_GetObjectItemCaseSensitive(json, "sensors");
...
void jsonWire() {
  ...
  if (root) {
    ...
    if (item)
      cJSON_AddItemToObject(root, "pin", item);
    item = cJSON_CreateNumber(queryInterval + DEVICE_RESPONSE_WAIT);
    if (item)
      cJSON_AddItemToObject(root, "interval", item);
    ...
    if (json) {
      ...
      for (sensor &dev : sens) {
        ...
        if (item)
          cJSON_AddItemToObject(entry, "C", item);
        item = cJSON_CreateNumber(dev.Ct);
        if (item)
          cJSON_AddItemToObject(entry, "Ct", item);
        item = cJSON_CreateNumber(dev.H);
        if (item)
          cJSON_AddItemToObject(entry, "H", item);
        item = cJSON_CreateNumber(dev.AF);
        if (item)
          cJSON_AddItemToObject(entry, "AF", item);
        item = cJSON_CreateNumber(dev.pin);
        if (item)
          cJSON_AddItemToObject(entry, "pin", item);
        item = cJSON_CreateNumber(dev.age);
        if (item)
          cJSON_AddItemToObject(entry, "seen", item);
...
void http1WireModify() {
  ...
  if (cJSON_HasObjectItem(root, "sensors"))
    json = cJSON_GetObjectItem(root, "sensors");
  if (json) {
